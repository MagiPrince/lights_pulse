...
static void dmx_task(void * arg)
{
    ...
    enum dmxQueue_t valQueue;
    while (1) {
        if(xQueueReceive( dmxQueue, &valQueue,portMAX_DELAY)){
            switch (valQueue) {
                case BEAT_MODE:
...
void createDmxTasks(){
    ...
    initDmx(dmx);
    dmxQueue = xQueueCreate( 10, sizeof(int) );
    xTaskCreate(dmx_task, "uart_tx_task", 1024*2, dmx, configMAX_PRIORITIES-1, NULL);
    xTaskCreate(dmx_send_task, "uart_tx_task", 1024*2, dmx, configMAX_PRIORITIES-1, NULL);
